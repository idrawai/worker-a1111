...
RUN wget -O /model.safetensors https://civitai.com/api/download/models/51913
...
RUN cd /stable-diffusion-webui && python cache.py --use-cpu=all --ckpt /stable-diffusion-webui/models/Stable-diffusion/model.safetensors && python cache.py --use-cpu=all
